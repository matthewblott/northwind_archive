namespace northwind.web.ui.tags
{
  ...
  [HtmlTargetElement("nw-field-select-enum")]
    public class FieldSelectEnumTagHelper : TagHelper
  {
    ...
    private TagBuilder GetSelectTag()
    {
      ...
      // todo: merge this with FieldSelectTagHelper
      var textValue = For.Model?.ToString();
      var enumValue = Items.Single(i => i.Text == textValue).Value;
      
      var any = Items.Any(x => x.Value == textValue);
      ...
      foreach (var item in Items)
      {
        ...
        option1.Attributes.AddIf(item.Value == enumValue, "selected"); 
