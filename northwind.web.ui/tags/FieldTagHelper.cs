...
namespace northwind.web.ui.tags
{
  ...
  public class FieldInputTagHelper : InputTagHelper
  {
    ...
    private bool IsDateTime() 
      => IsDataType() && GetModelAttributes().Foo<DataTypeAttribute>().DataType == DataType.DateTime;
    private bool IsDate() 
      => IsDataType() && GetModelAttributes().Foo<DataTypeAttribute>().DataType == DataType.Date;
    ...
    private TagBuilder GetInputTag()
    {
      ...
      input1.AddCssClassIf("validation", IsValidation);
      input1.Attributes.Add("id", For.Name);
      input1.Attributes.Add("name", For.Name);
      input1.Attributes.Add("value", $"{For.Model}");
      input1.Attributes.AddIf("readonly", IsReadOnlyResult());
      input1.Attributes.AddIf("required", IsRequired());
      
      input1.Attributes.Add("type", InputType.ToString().ToLower());

      var typeClassName = InputType == InputType.Text ? "input" : InputType.ToString().ToLower(); 
      
      input1.AddCssClass(typeClassName);
      
      if (IsRegularExpression())
      {
        input1.Attributes.Add("pattern", GetModelAttributes().Foo<RegularExpressionAttribute>().Pattern);
      }
      
      if (IsDate() && !IsReadOnlyResult())
      {
        input1.Attributes.Remove("type");
      ...
      if (IsCompare())
      {
        input1.Attributes.Add("data-nw-compare", GetModelAttributes().Foo<CompareAttribute>().OtherProperty);
      }
      
      if (IsEditable())
      {
        input1.Attributes.AddIf("readonly", !GetModelAttributes().Foo<EditableAttribute>().AllowEdit);
      }

      if (IsDisplay() && !IsDate() && !IsDateTime())
      {
        input1.Attributes.Add("placeholder", GetModelAttributes().Foo<DisplayAttribute>().Prompt);
      }

      if (IsDisplay() && IsDate())
      {
        input1.Attributes.Add("placeholder", DateTime.Today.ToShortTimeString());
      }

      if (IsDisplay() && IsDateTime())
      {
        input1.Attributes.Add("placeholder", DateTime.Today.ToShortTimeString());
      }
      ...
      if (!IsDisplay())
      {
        input1.Attributes.Add("placeholder", For.Name.Humanize().Titleize());
      }
      
      if (IsStringLength())
      {
        var attribute = GetModelAttributes().Foo<StringLengthAttribute>();
      ...
      if (IsMinLength() && input1.Attributes.All(x => x.Key != "minlength"))
      {
        input1.Attributes.Add("minlength", GetModelAttributes().Foo<MinLengthAttribute>().Length);
      }
      
      if (IsMaxLength() && input1.Attributes.All(x => x.Key != "maxlength"))
      {
        input1.Attributes.Add("maxlength", GetModelAttributes().Foo<MaxLengthAttribute>().Length);
      }

      if (IsRemote())
      {
        var attribute = GetModelAttributes().Foo<RemoteAttribute>();
        
        input1.Attributes.Add("data-nw-action", _urlHelper.ActionLink(attribute.Action, attribute.Controller));
        input1.Attributes.AddIf("data-nw-additional-fields",
          attribute.AdditionalFields, !string.IsNullOrWhiteSpace(attribute.AdditionalFields));
    ...
    private TagBuilder GetLabelTag()
    {
      ...
      var labelCaption = IsDisplay() 
        ? GetModelAttributes().Foo<DisplayAttribute>().Name : For.Name.Humanize().Titleize();
